 ...
 */
oj.ModuleBinding = {}; // Define a mapping variable that maps the return value of the module to the name used in the callback function of a require call.
...
(function () {
  ko.bindingHandlers.ojModule = {
    init: function init(element, valueAccessor, allBindingsAccessor, koViewModel, bindingContext) {
      ...
      var invokeModelDispose = function invokeModelDispose(model) {
        legacyViewModelMethodFunc(model, 'disposeMethod', [element, valueAccessor]);
      };

      var disposeAssociatedComponentViewModel = function disposeAssociatedComponentViewModel() {
        invokeModelDispose(currentViewModel);
      };

      var nodeDispose = function nodeDispose() {
        disposeAssociatedComponentViewModel(); // call dispose() on cached models
        ...
        resolveBusyState(); // Knockout will call ko.cleanNode on all child nodes including the cacheHolder
      };

      ko.utils.domNodeDisposal.addDisposeCallback(element, nodeDispose);

      var _IGNORE_PROMISE = new Error('Promise cancelled because ojModule is fetching new View and ViewModel'); // This function is used to interrupt Promise chains when the new view/viewModel combination is being loaded


      var checkPendingId = function checkPendingId(id) {
        if (id !== pendingViewId) {
          throw _IGNORE_PROMISE;
        }
      };

      var initCacheHolder = function initCacheHolder() {
        if (!cacheHolder) {
          cacheHolder = document.createElement('div');
          cacheHolder.className = 'oj-helper-module-cache'; // it is Ok to insert the cache holder as the first element because
      ...
      ko.computed(function () {
        ...
        if (!busyStateResolver) {
          ...
          busyStateResolver = busyContext.addBusyState({
            description: 'ojModule binding on a node with the Id ' + element.id + 'is loading the module. Binding evaluator: ' + valueAccessor.toString()
          });
        }
        ...
        if (modelPromise == null) {
          ...
          if (modelPromise != null) {
            ...
            modelPromise = modelPromise.then(function (id, viewModel) {
              checkPendingId(id);
              ...
              return viewModel;
            }.bind(null, pendingViewId)); // Handle the case where the Model is responsible for creating a View

            if (viewPromise == null && viewFunction != null) {
              viewPromise = modelPromise.then(function (id, model) {
                checkPendingId(id);
          ...
          if (viewPromise == null) {
            if (viewName != null) {
              ...
              viewPromise = _getOjModuleRequirePromise(requireFunc, 'view', viewPath + viewName + oj.ModuleBinding.defaults.viewSuffix);
            } else {
              resolveBusyState();
        ...
        if (modelPromise != null) {
          modelAttachPromise = modelPromise.then(function (id, viewModel) {
            checkPendingId(id);
        ...
        masterPromise.then(function (id, values) {
          if (id !== pendingViewId) {
            return;
          }
          ...
          var removeOldDomNodes = function removeOldDomNodes(oldViewParent) {
            if (oldNodesRemoved) {
              return;
            }
            ...
            if (saveInCache) {
              ...
              cache[currentCacheKey] = {
                ...
                view: cachedNodeArray
              };
            } else if (isCustomElement && currentCleanupMode === 'none') {
              _invokeOnSubtree(cachedNodeArray, oj.Components ? oj.Components.subtreeDetached : null);
            } else {
          ...
          function insertAndActivateNewNodes(_targetElement) {
            var targetElement = _targetElement || element; // For custom elements check whether the binding is already applied to nodes
            ...
            var fromCache = cached != null; // For upstream or indirect dependency we will still rely components being registered on the oj namespace.
            ...
            if (!fromCache && !bindingApplied) {
              var childBindingContext = bindingContext.createChildContext(model, undefined, function (ctx) {
                ctx.$module = model;
                ctx.$params = params;
              });
          ...
          if (animation != null) {
            ...
            var promise = _animate(actx, animation, element, oldDomNodes, insertAndActivateNewNodes, removeOldDomNodes, transitionComplete); // wrap currentAnimationPromise is a promise that never gets rejected, so that the ojModule can still
        ...
        function (id, reason) {
          ...
          if (reason != null) {
            resolveBusyState();
            Logger.error(reason); // Additionally log the stack trace for the original error
  ...
  function _animate(_actx, animation, element, oldDomNodes, insertAndActivateFunc, removeOldDomNodes, transitionComplete) {
    var actx = _actx;
    ...
    var insertNewView = function insertNewView() {
      if (viewInserted) {
        return;
      }
    ...
    var animationPromise = animation.animate.call(animation, actx).then(postAnimation, // success
    function () {
      // failure
      postAnimation();
  ...
  function _detachOldView(element, _oldViewParent, cacheHolder) {
    ...
    var empty = []; // If the node being emptied is the ojModule host node, ensure that the cacheHolder stays
  ...
  function _createNoFailPromise(promise) {
    ...
    var noFail = new Promise( // eslint-disable-next-line no-unused-vars
  ...
  function _checkForReusedView(element, nodes, model, isCustomElement, resolveBusyState) {
    ...
    if (isCustomElement && nodes.length > 0 && !_isBindingApplied(nodes, model)) {
      if (nodes[0]._oj_module_used_view) {
        ...
        Object.defineProperty(nodes[0], '_oj_module_used_view', {
          value: true
        });
      }
    }
