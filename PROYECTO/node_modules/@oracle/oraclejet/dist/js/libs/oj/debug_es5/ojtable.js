...
Table.prototype._refreshTableBody = function (resultObject, startIndex) {
  ...
  if (rows.length === 0 && tableBodyRows.length === 0) {
    ...
    tableBody.style[Table.CSS_PROP._VISIBILITY] = Table.CSS_VAL._HIDDEN; // clear cached dimensions to ensure table will be fully visible once render cycle completes
  ...
  if (rowRenderer != null || this._isDefaultRowTemplateSlotValid() && this._getSlotTemplate('rowTemplate') !== null) {
    ...
    if (this._columnsDestMap != null) {
      for (var _i2 = 0; _i2 < this._columnsDestMap.length; _i2++) {
        if (this._columnsDestMap[_i2] > _i2) {
          var moveTableBodyCell = tableBodyCells[this._columnsDestMap[_i2]];
          var swapTableBodyCell = tableBodyCells[_i2];
...
Table.prototype._refreshRow = function (rowIdx, resetFocus) {
  ...
  if (isNaN(rowIdx) || rowIdx < 0 || rowIdx >= tableBodyRows.length || tableBodyRows.length === 0) {
    ...
    var errDetail = Translations.applyParameters(Table._LOGGER_MSG._ERR_REFRESHROW_INVALID_INDEX_DETAIL, {
...
Table.prototype._afterRowsRendered = function (tableBody, delayRefresh) {
  this._clearCachedDomRowData(); // call a forced table size refresh here since we have rendered a bunch of rows
...
Table.prototype._getColumnIdxsForElementsWithStyleClass = function (styleClass) {
  ...
  if (elements && elements.length > 0) {
    ...
    for (var i = 0; i < elementsCount; i++) {
      ...
      if (columnIdx === null && elements[i].classList.contains(Table.CSS_CLASSES._COLUMN_HEADER_SELECTOR_CELL_CLASS)) {
        columnIdx = -1;
      }
...
Table.prototype._syncTableSizing = function (skipRequiredCheck) {
  ...
  this._refreshTableDimensions(null, null, skipRequiredCheck); // if loadMoreOnScroll then check if we have underflow and do a fetch if we do


  if (this._isLoadMoreOnScroll() && !this._dataFetching && this._domScroller) {
    this._setDataWaitingState(false);

    this._domScroller.checkViewport().then(this._domScrollerSuccessFunc, this._clearDataWaitingState.bind(this)); // syncScrollPosition would be invoked after data are fetched and new rows are rendered
...
Table.prototype._getRowsFromEventDetailAdd = function (eventDetail) {
  ...
  var afterKeyIter = eventDetail[Table._CONST_ADDBEFOREKEYS] ? eventDetail[Table._CONST_ADDBEFOREKEYS] : eventDetail[Table._CONST_AFTERKEYS];
  ...
  var maxRowIndex = rowCount - 1; // if specified, afterKeys takes precendence over indexes
  ...
  for (var i = 0; i < eventDataCount; i++) {
    ...
    if (!isNaN(eventIndex)) {
      rowIdx = eventIndex - offset; // for high watermark scrolling, we only care about rows in our currently fetched window
      ...
      if (this._isLoadMoreOnScroll()) {
        if (rowIdx <= maxRowIndex) {
          ...
          this._noMoreData = false; // eslint-disable-next-line no-continue
...
Table.prototype._updateAccStatusInfo = function (rowIndex, columnIndex) {
  ...
  if (rowIndex === -1) {
    ...
    label += tableHeaderColumn.id + ' ' + this._stateInfo.id; // apply new label
    ...
    this._stateInfo.textContent = stateInfo;
    label += this._stateInfo.id; // apply new label
...
Table.prototype._createContextMenu = function (handleContextMenuSelect) {
  ...
  } else if (this._isTableSortable() || enableNonContiguousSelectionMenu || this._isTableColumnsResizable()) {
    if (this._IsCustomElement()) {
...
Table.prototype._displayDragOverIndicatorRow = function (rowIdx, before, modelRow) {
  ...
  if (before) {
    tableBodyRowDragIndicator.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_BEFORE_CLASS);
  } else {
    tableBodyRowDragIndicator.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_AFTER_CLASS);
  }
...
Table.prototype._getScrollBarWidth = function () {
  return this._tableHeightConstrained && !this._isVerticalScrollBarHidden() ? this._getDefaultScrollBarSize() : 0;
};
/**
...
Table.prototype._isVerticalScrollBarHidden = function () {
  return !this._isTouchDevice() && this._getTableContainer().classList.contains(Table.MARKER_STYLE_CLASSES._HIDE_VERTICAL_SCROLLBAR);
};
/**
...
Table.prototype._insertTableFooterCell = function (columnIdx, tableFooterCell) {
  ...
  var tableFooterCells = this._getTableElementsByClassName(tableFooterRow, Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS);
...
Table.prototype._getTableContainerScrollableState = function (bottomSlotHeight) {
  ...
  if (tableContainer.clientHeight > 0) {
    var innerHeight = table.offsetHeight + bottomSlotHeight; // don't use tableContainer[0].scrollHeight as it may not be accurate
...
Table.prototype._removeDragOverIndicatorRow = function () {
  ...
  var indicatorRowBeforeElements = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_BEFORE_CLASS);

  var indicatorRowAfterElements = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_AFTER_CLASS);
...
Table.prototype._removeTableHeaderColumnDragImage = function () {
  ...
  var headerDragImage = this._getTableElementsByClassName(tableContainer, Table.CSS_CLASSES._COLUMN_HEADER_DRAG_IMAGE);
...
Table.prototype._isHorizontalGridEnabled = function () {
  return this.options.horizontalGridVisible === Table._OPTION_ENABLED || this.options.horizontalGridVisible === Table._OPTION_AUTO;
};
/**
...
Table.prototype._refreshTableDimensions = function (width, height, skipRequiredCheck) {
  ...
  var tableBottomSlot = this._getTableBottomSlot(); // preserve the scrollTop & scrollLeft


  var scrollTop = null;
  var scrollLeft = null;

  if (this._scrollTop != null && this._scrollTop > 0) {
    scrollTop = this._scrollTop;
  }

  if (this._scrollLeft != null && this._scrollLeft > 0) {
  ...
  this._styleTableContainer(tableContainer); // set the forced column widths before checking for scroll state, otherwise we
  // may miss cases where the columns[].width attribute values cause overflow


  this._setForcedColumnWidths(); // find Table's bottom slot height to use when determining if overflow if present
  ...
  if (this._tableHeightConstrained || this._tableWidthConstrained) {
    ...
    if (browserScrollBarSize > 0) {
      if (!this._tableWidthConstrained && !this._isVerticalScrollBarHidden()) {
        ...
        table.style[Table.CSS_PROP._WIDTH] = containerClientWidth + browserScrollBarSize + Table.CSS_VAL._PX;
        ...
        table.style[Table.CSS_PROP._HEIGHT] = containerClientHeight + browserScrollBarSize - bottomSlotHeight + Table.CSS_VAL._PX;
        ...
        if (tableContainer.clientHeight < containerClientHeight + browserScrollBarSize) {
          ...
          if (containerClientHeight - browserScrollBarSize < table.clientHeight + bottomSlotHeight) {
            this._tableHeightConstrained = true;
          }
        } else {
          containerClientHeight = tableContainer.clientHeight;
    ...
    var actualScrollBarWidth = this._getScrollBarWidth(); // now that overflow is determined, set the column widths
    ...
    if (tableHeader != null) {
      tableHeader.style[Table.CSS_PROP._WIDTH] = containerClientWidth - actualScrollBarWidth + Table.CSS_VAL._PX;
    }

    if (tableFooter != null) {
      tableFooter.style[Table.CSS_PROP._TOP] = tableBodyHeight + Table.CSS_VAL._PX; // if we have scrollbars then size the tablefooter to align with the scrollbars

      tableFooter.style[Table.CSS_PROP._WIDTH] = containerClientWidth - actualScrollBarWidth + Table.CSS_VAL._PX;
    }
  } else {
  ...
  if (scrollLeft != null) {
    // set flag so that scroll() will not cause an scrollPosition update
    this._skipScrollUpdate = true;
    oj.DomUtils.setScrollLeft(this._getScroller(), scrollLeft);
    $(this._getScroller()).scroll();
...
Table.prototype._refreshTouchAffordanceGlassPanePosition = function () {
  ...
  if (touchAffordanceGlassPane) {
    ...
    touchAffordanceGlassPane.style[Table.CSS_PROP._TOP] = tableBodyRect.top - tableContainerRect.top + Table.CSS_VAL._PX;
    touchAffordanceGlassPane.style[Table.CSS_PROP._BOTTOM] = tableBodyRect.bottom - tableContainerRect.bottom + Table.CSS_VAL._PX;
...
Table.prototype._removeTableDimensionsStyling = function () {
  ...
  if (tableHeader != null) {
    ...
    var headerColumnTextDivs = this._getTableElementsByClassName(tableHeaderRow, Table.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
...
Table.prototype._applyColumnHeaderHeight = function (headerColumnCell, height) {
  var headerColumnDiv = this._getTableElementsByClassName(headerColumnCell, Table.CSS_CLASSES._COLUMN_HEADER_CLASS);
...
Table.prototype._setForcedColumnWidths = function () {
  ...
  var tableBodyRows = this._getTableBodyRows(); // find and set the widths of the forced width columns, cells, and footers first
  ...
  for (var i = 0; i < columnsCount; i++) {
    ...
    if (columnWidth > 0) {
      ...
      if (typeof columnWidth !== 'number') {
        ...
        errDetail = Translations.applyParameters(Table._LOGGER_MSG._ERR_WIDTH_INVALID_TYPE_DETAIL, {
          width: columnWidth
        });
        Logger.warn(errSummary + '\n' + errDetail);
...
Table.prototype._setColumnWidths = function (scrollBarWidth) {
  ...
  var columnsCount = this._getColumnDefs().length; // loop through remaining columns to get the remaining column widths
  ...
  for (i = 0; i < columnsCount; i++) {
    if (this._forcedWidthColumns[i] !== false) {
      ...
      if (headerCell != null) {
        ...
        var headerTextDiv = this._getTableElementsByClassName(headerCell, Table.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
...
Table.prototype._events = {
  ...
  keydown: function keydown(event) {
    ...
    if (this._getKeyboardKeys().length === 1 || this._getKeyboardKeys().length === 2 && event[Table._KEYBOARD_CODES._MODIFIER_SHIFT]) {
      if (this._isKeyPressMatch(DataCollectionUtils.isArrowUpKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowDownKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isHomeKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isEndKeyEvent)) {
...
Table.prototype._setCurrentRow = function (currentRow, event, optionChange) {
  ...
  if (!isClearCurrentRow) {
    ...
    if (localRowIndex !== -1 && (!dataprovider || localRowIndex < -1 || localRowIndex === null || rowKey === null)) {
      ...
      errDetail = Translations.applyParameters(Table._LOGGER_MSG._ERR_CURRENTROW_UNAVAILABLE_INDEX_DETAIL, {
  ...
  if (currentRowChanged) {
    ...
    try {
      ...
      errDetail = Translations.applyParameters(Table._LOGGER_MSG._ERR_PRECURRENTROW_ERROR_DETAIL, {
        error: err.toString()
      });
...
Table.prototype._applyHeaderColumnState = function (headerColumn, columnIdx, state) {
  ...
  if (selected != null) {
    var headerColumnSelected = headerColumn.classList.contains(Table.MARKER_STYLE_CLASSES._SELECTED);
...
Table.prototype._setScrollX = function (x) {
  var scroller = this._getScroller(); // flag it so that handleScroll won't do anything
  ...
  if (tableHeaderRow != null) {
    ...
    if (this._headerScrollEventListener == null) {
      this._headerScrollEventListener = function (event) {
        var scrollLeft = this._getElementScrollLeft(event.target);
...
Table.prototype._handleKeydownEnter = function (event) {
  ...
  if (focusedColumnIdx != null && this._getColumnDefs()[focusedColumnIdx].sortable === Table._OPTION_ENABLED) {
...
Table.prototype._NotifyAttached = function () {
  this._super(); // refresh dimensions if no tasks are pending since refresh dimensions runs during final task
...
oj.TableDndContext.prototype._removeDragMarkerClass = function () {
  var dragColumns = this.component._getTableElementsByClassName(this.component._getTableHeader(), Table.MARKER_STYLE_CLASSES._DRAG);
...
oj.TableDndContext.prototype._cloneTableContainer = function (tableContainer) {
  ...
  var tableBodyClone = this.component._getTableElementsByTagName(tableContainerClone, Table.DOM_ELEMENT._TBODY, true)[0];

  if (this.component._isDefaultSelectorEnabled()) {
    var headerSelectorElements = this.component._getTableElementsByClassName(tableContainerClone, Table.CSS_CLASSES._TABLE_HEADER_SELECTOR_CLASS, true);
    ...
    var selectors = this.component._getTableElementsByClassName(tableContainerClone, Table.CSS_CLASSES._TABLE_DATA_ROW_SELECTOR_CLASS, true);
  ...
  tableContainerClone.style[Table.CSS_PROP._TOP] = -10000 + Table.CSS_VAL._PX; // Set both overflow and overflow-x/y because on IE, setting overflow does not
...
oj.TableDndContext.prototype.handleColumnReorderDrop = function (event) {
  ...
  this.component._columnsDestMap = this.component._moveTableHeaderColumn(this._dragStartColumnIdx, columnIdx, event);
  event.preventDefault();
};
/**
...
oj.TableDndContext.prototype._hideUnselectedRows = function (tableContainerClone, selArray, rowDataArray) {
  var i;
  var j;
  var firstRow = null;

  var tableBodyRows = this.component._getTableElementsByClassName(tableContainerClone, Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS, true);
  ...
  for (i = tableBodyRows.length - 1; i >= 0; i--) {
    ...
    if (selectedRowIndex === -1) {
      ...
      var cells = this.component._getTableElementsByClassName(row, Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);
  ...
  var tableHeader = this.component._getTableElementsByTagName(tableContainerClone, Table.DOM_ELEMENT._THEAD, true);
  ...
  if (tableHeader != null && tableHeader.length > 0) {
    ...
    var headerCells = this.component._getTableElementsByClassName(tableHeader[0], Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
  ...
  var tableFooter = this.component._getTableElementsByTagName(tableContainerClone, Table.DOM_ELEMENT._TFOOT, true);
...
oj.TableDndContext.prototype._setDragRowsDataTransfer = function (event, dataTypes, selArray) {
  ...
  if (rowDataArray.length) {
    ...
    this._dragImage = this._setDragRowsImage(event.originalEvent, $(this.component._getFirstAncestor(event.currentTarget, Table.DOM_ELEMENT._TABLE, true)).parent()[0], selArray, rowDataArray);
    return {
      rows: rowDataArray
    };
  }
...
oj.TableDndContext.prototype._setDragRowsImage = function (nativeEvent, tableContainer, selArray, rowDataArray) {
...
oj.TableDndContext.prototype._setReorderColumnsDataTransfer = function (event, columnIdx) {
  ...
  dataTransfer.setData('Text', Table._DND_REORDER_TABLE_ID_DATA_KEY + ':' + tableIdHashCode + ':' + columnIdx);
