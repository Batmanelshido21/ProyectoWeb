...
/* global ko:false, BindingProviderImpl: false, WeakMap: false, Map: false, HtmlUtils:false, Logger: false*/
...
function JetTemplateEngine() {
  ...
  this.execute = function (componentElement, node, properties, alias) {
    var tmpContainer = _createAndPopulateContainer(node); // Check to see if data-oj-as was defined on the template element as an additional
    // alias to provide to the template children


    var templateAlias = node.getAttribute('data-oj-as');
    ko.applyBindingsToDescendants(_getContext(componentElement, node, properties, alias, templateAlias), tmpContainer);
  ...
  function _getPropertyEvaluatorMap(firstElem, propertySet, context) {
    ...
    for (var i = 0; i < attrs.length; i++) {
      ...
      var prop = oj.__AttributeUtils.attributeToPropertyName(attr.name); // Handle the 'dot' notation for bound subprops
