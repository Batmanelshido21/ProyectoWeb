 ...
 */
oj.ComponentMessaging.prototype._getResolvedMessagingDisplayOptions =
...
oj.ComponentMessaging.prototype._getUserAssistanceStrategyToArtifactsObj = function () {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (typeof strategyTypes === 'string' && strategyTypes === 'none') {
      ...
      oj.ComponentMessaging._STRATEGY_TYPE.NONE;
    } else {
  ...
  Object.keys(oj.ComponentMessaging._STRATEGY_TYPE).forEach(function (key) {
    let value = oj.ComponentMessaging._STRATEGY_TYPE[key];
...
oj.ComponentMessaging.prototype._resolveStrategyTypeForArtifact = function (
  artifact,
  strategyType,
  options,
  $messagingPreferences) {
...
oj.ComponentMessaging.prototype._initializeMessagingStrategies = function () {
  ...
  var displayInPlaceholder =
      strategyToArtifacts[oj.ComponentMessaging._STRATEGY_TYPE.PLACEHOLDER];
  var displayInline = strategyToArtifacts[oj.ComponentMessaging._STRATEGY_TYPE.INLINE];
  // these are input components
  var displayLabelEdgeInsideTextField =
    strategyToArtifacts[oj.ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE];
  // these are form controls that aren't inputs, like radioset
  var displayLabelEdgeInsideFormControl =
    strategyToArtifacts[oj.ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE_FORM_CNTRL];
  // these are for user assistance inline that is used in the Redwood theme
  var displayUserAssistanceInline =
  strategyToArtifacts[oj.ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE];
  ...
  if (displayUserAssistanceInline.length > 0) {
    messagingStrategies[oj.ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE] =
