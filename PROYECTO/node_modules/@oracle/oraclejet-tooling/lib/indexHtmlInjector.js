...
function _getStyleLinkBase(css, theme, cssvars, buildType) {
...
function _getThemeStyleLink(theme, buildType, csstype) {
  const css = config('paths').src.styles;
  const cssvarsValue = csstype;
  const cssVarsDestPath = path.resolve(config('paths').src.common, 'themes', theme.name, 'web', `_${theme.name}.cssvars.settings.scss`);
  const pcssTheme = fs.existsSync(util.destPath(cssVarsDestPath));
  const cssvars = (() => {
    if (cssvarsValue === 'enabled') {
      if (theme.name === 'redwood' || pcssTheme) {
        return '-cssvars';
      }
    }
    if (cssvarsValue === 'disabled') {
      return '';
    }
    return util.log.error('In oraclejetconfig.json value of defaultCssvars must be [enabled or disabled]\nor\ndefaultCssvars:"enabled" supported only for pcss themes, not supported for alta themes');
  })();
  const linkBase = _getStyleLinkBase(css, theme, cssvars, buildType);
...
module.exports =
{
  ...
  injectThemePath: (context) => {
    ...
    const csstype = (opts.defaultCssvars) ? opts.defaultCssvars : config('defaultCssvars');
    const themeStyleLink = _getThemeStyleLink(theme, buildType, csstype);
